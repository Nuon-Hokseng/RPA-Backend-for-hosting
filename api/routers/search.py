...
def _search_worker(task_id: str, account_path: str, search_term: str,
                   search_type: str, headless: bool, keep_open: bool):
    log = make_log_fn(task_id)
    stop = make_stop_fn(task_id)
    update_task(task_id, status=TaskStatus.RUNNING)
    log(f"Search – term='{search_term}', type={search_type}, headless={headless}")
...
async def run_search(req: SearchRequest, bg: BackgroundTasks):
    ...
    bg.add_task(
        _search_worker,
        task.task_id, req.account_path, req.search_term,
        req.search_type, req.headless, req.keep_open,
    )
    return TaskResponse(
